...
我有时候会想到1些feature来提高SDXL Lora的训练效果，但是去 fork 别人的仓库来改又太笨重了，所以就自己写了1个！
...
实现方式也非常简单，直接把所有的模型都放在RAM里，然后隔几个step把它们轮流传输到VRAM。

但也有观众可能会想，那怎么证明它不是只适应了训练时的N的模型呢？
...
而且流程也比较简单，不用提前生成正则化图片，甚至还可以用训练集把触发词去掉直接正则自己，效果也很好。
...
还有我突然想到一件事，但如果1个mask不是二值的话，那它岂不就成了veil？


### Timestep区间

训练画风的时候，会担心画师的构图不小心被学到。因此就加上了限制训练时的timestep的功能，这样就可以自定义要丢掉高噪部分或是低噪部分了。
...
| `cache_dir` | `str` | `"./rimo_lora_lab_cache"` | VAE Latent 缓存目录 |
| `resume_from_checkpoint` | `str` | `"latest"` | 恢复训练的 checkpoint 路径 |
...
| `checkpointing_steps` | `int` | `500` | 保存 checkpoint 的步数间隔 |
...
| `gradient_checkpointing` | `bool` | `False` | 开启梯度检查点以节省显存 |
| `mixed_precision` | `str` | `None` | 混合精度模式（`"fp16"`、`"bf16"` 或 `None`） |
...
| `adam_beta1` / `adam_beta2` | `float` | `0.9` / `0.999` | Adam 系列优化器超参数 |
| `adam_weight_decay` | `float` | `0.1` | 权重衰减系数 |
| `max_grad_norm` | `float` | `1.0` | 梯度裁剪阈值 |

- `optimizer`可以选`adam`、`8bit_adam`、`prodigy`、`muon`。
...
| `loss_type` | `str` | `"l2"` | 损失函数类型（`"l2"`、`"huber"`、`"huber_scheduled"`） |
| `huber_c` | `float` | `0.1` | Huber 损失平滑因子 |
| `snr_gamma` | `float` | `None` | Min-SNR 加权系数 |
| `time_min` / `time_max` | `int` | `0` / `1000` | 扩散时间步区间限制 |

### 数据处理
| 参数 | 类型 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `size_min` / `size_max` | `int` | `704` / `1280` | 随机缩放分辨率范围 |
| `drop_tag_rate` | `float` | `0.0` | 逗号分隔 tag 的随机丢弃概率 |
| `drop_text_rate` | `float` | `0.0` | 提示词整体随机置空概率 |
| `prompt_post_process` | `str` | `""` | prompt后处理 |
| `prior_loss_prompt_post_process` | `str` | `""` | 先验正则化的prompt后处理 |
| `use_mask` | `bool` | `False` | 是否开启 Mask 区域加权损失 |
| `mask_min` | `float` | `0.1` | Mask 以外非重点区域的最小损失权重 |
